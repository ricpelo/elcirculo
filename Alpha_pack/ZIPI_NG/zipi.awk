...
/<PREG>/{
  ...
  m=menunivel[nivel];
  if (opciones[m]==32) {
    print "ERROR L" NR ": Más de 32 pistas en un menu.">"/dev/stderr";
    exit(-1);
  }
...
}
{
  ...
  if (!($0~("^[ \t]*$"))) {
	print "WARNING: la línea "NR" será considerada continuación de la línea anterior" \
	> "/dev/stderr";
      for (i=1;i<NF;i++)
